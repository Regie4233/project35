...
public struct MarchingCubesJob : IJob
{
    ...
    public void Execute()
    {
        // Standard Paul Bourke Corner Order
        ...
        cornerOffsets[2] = new float3(1, 0, 1);
        cornerOffsets[3] = new float3(0, 0, 1);
        cornerOffsets[4] = new float3(0, 1, 0);
        cornerOffsets[5] = new float3(1, 1, 0);
            ...
            for (int y = 0; y < ChunkSize.y; y++)
            {
                for (int x = 0; x < ChunkSize.x; x++)
                {
                    ...
                    for (int i = 0; TriTable[cubeIndex * 16 + i] != -1; i += 3)
                    {
                        ...
                        // Use standard winding for standard table
    ...
    private float3 GetGradientNormal(float3 pos)
    {
        ...
        if (math.lengthsq(gradient) < 0.0001f) return new float3(0, 1, 0);
